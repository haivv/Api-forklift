...
router.get('/user', (req, res) => {
	...
	var query1 = `SELECT * FROM account ORDER BY idAcc DESC`;
...
router.get('/delete/:idAcc', (req, res) => {
	var idAcc = req.params.idAcc;
	...
	try {
		jwt.verify(accessToken, accesssecretKey, (err, decoded) => {
			if (err) {
				...
				const queryDelUser = `DELETE FROM account WHERE idAcc="${idAcc}"`;
				database.query(queryDelUser, function (error, dataDelUser) {
					if (error) {
						console.error('Error deleting user:', error);
						res.status(500).json({ message: 'Internal server error' });

					} else {
						if (dataDelUser.affectedRows === 0) {
							delResult = { message: 'Delete fail' };
							res.status(401).json({ message: 'Delete fail' });

						} else {
							delResult = { message: 'Delete success' };
